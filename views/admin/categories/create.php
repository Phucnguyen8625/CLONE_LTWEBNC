...
<div class="max-w-2xl">
    <form action="admin.php?controller=category&action=store" method="POST" class="space-y-8">
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            <div class="p-6 border-b border-slate-100 flex items-center">
                ...
                <h3 class="text-sm font-black text-slate-800 uppercase tracking-widest">Thông tin danh mục</h3>
            </div>
            ...
            <div class="p-8 space-y-6">
                <div>
                    ...
                    <input type="text" name="name" required class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-accent transition" placeholder="Ví dụ: Hành động, Tình cảm...">
                </div>

                <div>
                    <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 block">Mô tả thể loại</label>
                    <textarea name="description" rows="4" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-accent transition" placeholder="Mô tả tóm tắt về thể loại này..."></textarea>
                </div>

                <div class="flex items-center space-x-3 p-4 bg-slate-50 rounded-xl border border-slate-100">
                    <input type="checkbox" id="status" name="status" value="1" checked class="w-5 h-5 rounded border-slate-300 text-accent focus:ring-accent transition cursor-pointer">
